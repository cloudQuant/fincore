...
- 风险验证报告：`walk_forward_var` 返回 `WalkForwardVaRResult`；调用 `build_risk_validation_report` 可生成 `RiskValidationReport` JSON 台账，逐条保留 out-of-sample forecast、realized return、exception、refit 参数、输入 digest 与 backtest digest；能力清单明确标注为 experimental，Basel traffic-light 字段只作参考，不宣称监管批准。
...
| Task 10 Fama–MacBeth 对齐补充（2026-08-22） | 单行静态暴露会按 return dates 广播，时变暴露按资产标签而非输入列位置对齐；新增独立 statsmodels OLS cross-sectional oracle、静态截面和乱序资产 adversarial fixtures。`tests/numerical/test_factor_inference.py tests/test_factor_analysis tests/compat/alphalens`：1,076 passed、61 warnings；Ruff、Mypy 通过。该修复只收敛既有 Fama–MacBeth 例程；其 Newey–West 补充列于下一行，仍不构成 PIT、FDR、cluster SE、成本或容量工作流的完成证据。 |
| Task 10 Fama–MacBeth Newey–West 补充（2026-08-22） | `fama_macbeth` 保持 i.i.d. 默认行为，并新增显式 `covariance="newey-west"`/`newey_west_lags`：对按时间顺序保留的截面 intercept 与 slope 序列计算 Bartlett HAC 标准误，结果 attrs 写入 covariance、lag 和 fitted cross-section count。非时间顺序、无效 lag 或 lag 不小于实际 fitted count 都 fail closed。独立 `statsmodels.OLS(..., cov_type="HAC")` oracle 在 0/1/3/8 个 lag 下验证两条系数序列的 mean/SE，静态暴露、资产标签乱序与默认 i.i.d. 兼容测试保留。完整 Task 10 因子/归因/优化/Alphalens 集合：1,419 passed、62 warnings，463.14s；Fama/API/capability/docs focused 集合：79 passed、1 warning。该切片不提供 clustered/multi-factor covariance、自动 lag selection、trials 或报告整合。 |
| Task 10 Benjamini–Hochberg FDR 补充（2026-08-22） | 新增可审计的独立推断函数：保留唯一 factor label、返回原始 p 值、BH adjusted p-value 与 step-up 决策；无效概率、重复标签及无效 `alpha` fail closed，空输入返回显式空结果。它与 `statsmodels.stats.multitest.multipletests(method="fdr_bh")` 的乱序/tie fixture 一致。`tests/numerical/test_factor_inference.py tests/test_factor_analysis tests/compat/alphalens`：1,085 passed、61 warnings；Ruff、Mypy 通过。后续 post-analysis 接入列于下一行；它仍未覆盖所有 enhanced 报告或研究者 trials 记录。 |
...
| Task 10 enhanced IC/FDR post-analysis 补充（2026-08-22） | 新增 `factor_model_inference(model, alpha=...)`：只消费 `FactorAnalysisModel` 已存的 aggregate IC snapshot，不重新计算因子收益/权重；逐 horizon 记录有限样本数、mean IC、two-sided Student-t p-value、BH q-value、rejection 及 `testable`。不足两个有限 IC 的 horizon 保留为不可检验，不能被伪装成无发现。p-value 与 `scipy.stats.ttest_1samp`、BH 结果与 statsmodels oracle 一致；public capability 标记为 experimental。`tests/numerical/test_factor_inference.py tests/numerical/test_factor_model_inference.py tests/numerical/test_factor_pit_materialization.py tests/numerical/test_optimization_feasibility.py tests/test_factor_analysis tests/test_attribution tests/test_optimization tests/compat/alphalens`：1,401 passed、61 warnings，759.26s；capability/API snapshot focused：36 passed、1 warning；Ruff、Mypy、capability snapshot 与 `mkdocs build --strict` 通过（MkDocs/Material 给出上游迁移警告）。这是 i.i.d. post-analysis，不构成 HAC/cluster、pre-registration/trial registry、成本/容量或报告级披露的完成证据。 |
...
| Task 10 多期限可用性补充（2026-08-22） | enhanced 层新增 `prepare_factor_data_by_horizon` 与只读 `MultiHorizonPreparedFactorData.by_horizon`：每个 computed horizon 单独保留有限收益、单独统计 `FactorLossReport` 并单独执行 `max_loss`；长期收益缺失不再删除短期有效样本。量化在有限 factor/universe panel 上先完成，终端价格只影响长期收益时不得改写短期表；重复 horizon label 与全样本 `filter_zscore` fail closed。strict Alphalens 的全列 source-shaped 清洗未改。`tests/numerical/test_factor_inference.py tests/numerical/test_factor_model_inference.py tests/numerical/test_factor_pit_materialization.py tests/numerical/test_factor_multihorizon_preparation.py tests/numerical/test_optimization_feasibility.py tests/test_factor_analysis tests/test_attribution tests/test_optimization tests/compat/alphalens`：1,406 passed、62 warnings，442.06s。它只完成可用性隔离，不替代 corporate-action/calendar provenance、成本、borrow、slippage、容量、trial registry 或完整 factor workflow 验收。 |
| Task 10 成本、借券与容量账本补充（2026-08-22） | enhanced 层新增 `apply_factor_costs`、`estimate_factor_capacity`、`FactorCostModel` 与不可变 `FactorCostResult`：从零初始头寸及逐次权重变化计算 turnover、spread、temporary-impact slippage、short borrow、gross-to-net return、participation 和 binding capacity。短仓必须具备完整的逐期 borrow rate/availability，缺失数据、借券不可用、超参与率及算术溢出均 fail closed；稀疏权重退出按零头寸真实计为交易。NumPy-only 独立 oracle 对账所有账本列和 capacity，并覆盖资产/日期标签重排、稀疏退出、不可借券、超容量和不可变快照。`tests/numerical/test_factor_inference.py tests/numerical/test_factor_model_inference.py tests/numerical/test_factor_pit_materialization.py tests/numerical/test_factor_multihorizon_preparation.py tests/numerical/test_factor_costs.py tests/numerical/test_optimization_feasibility.py tests/test_factor_analysis tests/test_attribution tests/test_optimization tests/compat/alphalens`：1,413 passed、62 warnings，381.70s；成本/API/capability/docs focused 集合：50 passed、1 warning；Ruff、Mypy、capability/API snapshot 与 `mkdocs build --strict` 通过（MkDocs/Material 给出上游迁移警告）。该切片是显式研究会计约定，不构成 calibrated execution、FX/liquidity/borrow source provenance、自动接入所有 factor report 或完整 Task 10 验收。 |
| Task 10 Brinson--Hood--Beebower 公开选项补充（2026-08-22） | `BrinsonAttribution.calculate(method="brinson_hood")` 不再公开抛 `NotImplementedError`；它是经公式验证的 BHB 单期分解兼容别名。独立 NumPy oracle 对每期 allocation、selection、interaction、total 和组合/基准收益逐项对账，既有 Carino 多期对账保持不变。完整 Task 10 因子/归因/优化/Alphalens 集合：1,420 passed、62 warnings，334.27s；BHB capability/API/docs/snapshot focused 集合：81 passed、1 warning；Ruff、Mypy、`mkdocs build --strict`、capability/API snapshot check 通过（MkDocs/Material 给出上游迁移警告）。该变更不新增另一套归因公式，也不把现有 class-level equal-weight benchmark 约定伪称为可配置 benchmark-weight 工作流。 |
...
3. **计划中的核心工作流仍未完成验收。** 现金流语义已接入增强报告与 manifest，但这不把它升级为完整的多币种报告工作流或 GIPS 认证；`fincore/risk/report.py` 已为现有 one-step walk-forward VaR 提供可复现的 reference 报告，却不覆盖 GARCH/EVT 的完整 out-of-sample calibration、convergence/residual/parameter-uncertainty 诊断或监管认证。PIT 因果物化、逐 horizon 可用性隔离、独立成本/borrow/capacity 会计账本和 Fama–MacBeth Newey–West 标准误已接入 enhanced 边界，但 corporate-action/calendar/liquidity/borrow provenance、执行参数校准、cluster/multi-factor covariance、研究 trial 记录和自动接入完整 factor workflow/report 仍未验收；IC/FDR 仍只作 i.i.d. 推断。该局部账本/推断补充不能使其余 T9–T10 或整个计划视为完成。
...
3. 扩展并独立验收剩余风险模型（GARCH/EVT 的 out-of-sample calibration、convergence/residual/parameter-uncertainty）、PIT 的 corporate-action/calendar provenance 与完整 enhanced workflow 整合、FDR 的报告/trials 记录、Fama–MacBeth cluster/multi-factor covariance 与 lag policy、成本/容量参数的流动性与借券来源/FX/执行校准，以及优化 KKT/残差等 T9–T10 项；每项需 oracle、property 与 adversarial fixture。绩效报告后续若要支持完整多币种台账，必须以带估值和 FX provenance 的端到端工作流验收，不能把 `DisclosureContext` 当作计算证据。
