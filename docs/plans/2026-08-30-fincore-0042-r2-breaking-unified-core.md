...
7A. `capture_capability_baseline.py` 只冻结 initial-HEAD 输入、fixture manifest 与 tooling provenance；它本身不得凭 ledger 推断或写入 capability `status=ok`。新增冻结的 scenario evaluator，在 clean D0 source 中逐项执行 ledger 所绑定的 golden/oracle/invariant 与 `source_nodeids`，输出带 `capabilities` 映射的 evaluated baseline；每个 `ok` 必须同时绑定 input-capture SHA、ledger SHA、scenario ID、实际 argv/exit、output/golden digest 与 D0 source identity。`wheel_nodeids` 作为同一 capability 的冻结候选-wheel contract 记录，必须由后续 detached acceptance runner 对同一候选 wheel 单独执行和绑定，不能在 D0 source evaluator 中伪造 wheel 通过。任一 D0 条目未执行、无 authority、无 output digest 或仅由 candidate 声明时，输出 `pending` 并使 parity `BLOCKED`，不得降格为 PASS/FAIL 的伪结论。
...
- required capability 的适用 happy/boundary/error/optional/provider/documented scenario 覆盖率为 100%，且 evaluated baseline 中每个 `ok` 都能回溯 input-capture SHA、ledger SHA、scenario authority、实际 source 执行、source identity 与输出 digest；后续 candidate wheel 证据必须对相同 capability 的冻结 `wheel_nodeids` 单独闭环。
...
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python -I "$FINCORE_0042R2_D0_TOOLING_ROOT/scripts/run_0042_r2_acceptance.py" --candidate-root "$FINCORE_0042R2_CANDIDATE_ROOT" --candidate-head "$FINCORE_0042R2_CANDIDATE_HEAD" --candidate-wheel "$FINCORE_0042R2_WHEEL" --expected-bundle "$FINCORE_0042R2_D0_BUNDLE" --gate quality --require-fresh-coverage --require-changed-lines 95 --require-critical-branches 90 --output-dir "$FINCORE_0042R2_FINAL_EVIDENCE/quality"
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python -I "$FINCORE_0042R2_D0_TOOLING_ROOT/scripts/run_0042_r2_acceptance.py" --candidate-root "$FINCORE_0042R2_CANDIDATE_ROOT" --candidate-head "$FINCORE_0042R2_CANDIDATE_HEAD" --candidate-wheel "$FINCORE_0042R2_WHEEL" --candidate-dist "$FINCORE_0042R2_FINAL_DIST" --expected-bundle "$FINCORE_0042R2_D0_BUNDLE" --gate parity --families all --require-source-wheel-equal --output-dir "$FINCORE_0042R2_FINAL_EVIDENCE/parity"
...
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python -I "$FINCORE_0042R2_D0_TOOLING_ROOT/scripts/run_0042_r2_acceptance.py" --candidate-root "$FINCORE_0042R2_CANDIDATE_ROOT" --candidate-head "$FINCORE_0042R2_CANDIDATE_HEAD" --candidate-wheel "$FINCORE_0042R2_WHEEL" --candidate-dist "$FINCORE_0042R2_FINAL_DIST" --expected-bundle "$FINCORE_0042R2_D0_BUNDLE" --gate installed --profiles core factor-analysis visualization report-pdf report-xlsx bayesian all --data-providers all --dependency-lanes minimum latest --output-dir "$FINCORE_0042R2_FINAL_EVIDENCE/installed"
