...
its two frozen upstream targets. Everything claimed here is enforced by
executable gates in `tests/compat/` (CI job `compat`) against the frozen
manifests in `tests/compat/fixtures/` — nothing is asserted by hand.
...
- Frozen JSON manifests: `tests/compat/fixtures/empyrical-0.6.0-api.json`,
  `pyfolio-0.9.6-api.json`, `fincore-flat-api-migrations.json` — generated by
  `scripts/generate_compat_manifest.py` from the pinned upstream commits.
- Executable suites: `tests/compat/empyrical/` and `tests/compat/pyfolio/`.
- Manifest integrity: `tests/compat/test_manifest_integrity.py`.
- CI job: `compat` in `.github/workflows/ci.yml`.
