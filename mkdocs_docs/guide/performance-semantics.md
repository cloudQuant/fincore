...
`DatetimeIndex`. A positive external cashflow is a contribution into the
portfolio; a negative one is a withdrawal. Every flow must have the same date
as a valuation because an unvalued intra-period flow has no defensible timing.
...
infers it from a timestamp alone.
