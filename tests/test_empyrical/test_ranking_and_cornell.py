...
class TestRankingAndCornell(TestCase):
    ...
    def test_alpha_percentile_rank(self):
        ...
        result = emp.alpha_percentile_rank(
            self.strategy_returns, list(self.all_strategies.values()), self.market_returns
        )
    ...
    def test_alpha_percentile_rank_best(self):
        ...
        emp = Empyrical()
        result = emp.alpha_percentile_rank(best_returns, others, self.market_returns)
    ...
    def test_alpha_percentile_rank_worst(self):
        ...
        emp = Empyrical()
        result = emp.alpha_percentile_rank(worst_returns, others, self.market_returns)
    ...
    def test_alpha_percentile_rank_empty(self):
        ...
        emp = Empyrical()
        result = emp.alpha_percentile_rank(empty_returns, list(self.all_strategies.values()), self.market_returns)
    ...
    def test_cornell_timing(self):
        ...
        emp = Empyrical()
        result = emp.cornell_timing(self.strategy_returns, self.market_returns)
    ...
    def test_cornell_timing_empty(self):
        ...
        empty_returns = pd.Series([], dtype=float)
        emp = Empyrical()
        result = emp.cornell_timing(empty_returns, self.market_returns)
        assert np.isnan(result)
    ...
    def test_cornell_timing_values(self):
        ...
        timing_returns = pd.Series(
            np.random.randn(500) / 100 + 0.0003 + timing_signal, index=pd.date_range("2020-1-1", periods=500, freq="D")
        )
