"""Independent contract test for a candidate's 0042-R2 matrix workflow.

The break this test catches is a workflow that looks cross-platform but
rebuilds a wheel per cell, omits an OS, or runs mutable candidate tooling.  The
frozen acceptance runner supplies ``FINCORE_0042R2_SOURCE_ROOT`` so this test
always reads the candidate workflow rather than the tooling checkout.
"""
...
SOURCE_ROOT = os.environ.get("FINCORE_0042R2_SOURCE_ROOT")
if not SOURCE_ROOT:
    pytestmark = pytest.mark.skip(reason="requires FINCORE_0042R2_SOURCE_ROOT")

WORKFLOW_PATH = Path(SOURCE_ROOT or ".") / ".github" / "workflows" / "ci.yml"
...
def test_r2_matrix_cells_download_the_single_wheel_and_run_the_frozen_contract() -> None:
    ...
    assert "r2-build-handoff.json" in cell_run
    assert upload["uses"] == "actions/upload-artifact@v4"
...
def test_r2_aggregate_only_consumes_matrix_cell_artifacts() -> None:
    ...
    assert "r2-build-handoff.json" in aggregate_run
