...
class PerfAttribTestCase(unittest.TestCase):
    ...
    def test_perf_attrib_regression(self):
        ...
        risk_exposures_portfolio.equals(pd.DataFrame(np.ones_like(risk_exposures_portfolio),
                         index=risk_exposures_portfolio.index,
                         columns=risk_exposures_portfolio.columns))
        ...
        self.assertEqual(Empyrical.annual_return(specific_returns),
                         perf_attrib_summary['Annualized Specific Return'])

        self.assertEqual(Empyrical.annual_return(common_returns),
                         perf_attrib_summary['Annualized Common Return'])

        self.assertEqual(Empyrical.annual_return(combined_returns),
                         perf_attrib_summary['Annualized Total Return'])

        self.assertEqual(Empyrical.sharpe_ratio(specific_returns),
                         perf_attrib_summary['Specific Sharpe Ratio'])

        self.assertEqual(Empyrical.cum_returns_final(specific_returns),
                         perf_attrib_summary['Cumulative Specific Return'])

        self.assertEqual(Empyrical.cum_returns_final(common_returns),
                         perf_attrib_summary['Cumulative Common Return'])

        self.assertEqual(Empyrical.cum_returns_final(combined_returns),
                         perf_attrib_summary['Total Returns'])
        ...
        cumulative_returns_by_factor = pd.Series(
            [Empyrical.cum_returns_final(perf_attrib_output[c])
        ...
        annualized_returns_by_factor = pd.Series(
            [Empyrical.annual_return(perf_attrib_output[c])
             for c in risk_exposures_portfolio.columns],
