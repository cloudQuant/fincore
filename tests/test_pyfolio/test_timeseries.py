...
from fincore.utils.common_utils import get_month_end_freq, to_series, to_utc
...
class TestDrawdown(TestCase):
    ...
    def test_get_max_drawdown_begins_first_day(self, px):
        rets = px.pct_change()
        drawdowns = Empyrical.gen_drawdown_table(rets, top=1)
        self.assertEqual(drawdowns.loc[0, "Net drawdown in %"], 25)

    drawdown_list = (
        np.array([100, 110, 120, 150, 180, 200, 100, 120, 160, 180, 200, 300, 400, 500, 600, 800, 900, 1000, 650, 600])
        / 10.0
    )
    ...
    def test_gen_drawdown_table_relative(
        ...
        drawdowns = Empyrical.gen_drawdown_table(rets, top=2)
    ...
    def test_get_max_drawdown(self, px, expected_peak, expected_valley, expected_recovery):
        ...
        peak, valley, recovery = Empyrical.get_max_drawdown(rets)
    ...
    @parameterized.expand(
        [
            (pd.Series(px_list_1, index=dt), 1, []),
        ]
    )
    def test_top_drawdowns(self, returns, top, expected):
        self.assertEqual(Empyrical.get_top_drawdowns(returns, top=top), expected)
...
class TestVariance(TestCase):
    ...
    def test_var_cov_var_normal(self, p, c, mu, sigma, expected):
        self.assertEqual(Empyrical.var_cov_var_normal(p, c, mu, sigma), expected)
...
class TestNormalize(TestCase):
    ...
    def test_normalize(self, returns, expected):
        self.assertTrue(Empyrical.normalize(returns).equals(expected))
...
class TestStats(TestCase):
    ...
    simple_month_rets = pd.Series(
        [0.1] * 3 + [0] * 497, pd.date_range("2000-1-31", periods=500, freq=get_month_end_freq())
    )
    ...
    @parameterized.expand([(simple_rets[:5], 2, [np.nan, np.inf, np.inf, 11.224972160321, np.nan])])
    def test_sharpe_2(self, returns, rolling_sharpe_window, expected):
        print("returns = ", returns)
        result = Empyrical.rolling_sharpe(returns, rolling_sharpe_window).values
...
class TestCone(TestCase):
    def test_bootstrap_cone_against_linear_cone_normal_returns(self):
        ...
        bootstrap_cone = Empyrical.forecast_cone_bootstrap(
            rets, days_forward, cone_stdevs, starting_value=1, random_seed=random_seed, num_samples=10000
        )
...
class TestBootstrap(TestCase):
    ...
    def test_calc_bootstrap(self, true_mean, true_sd, n):
        ...
        samples = Empyrical.calc_bootstrap(func, returns, n_samples=10000, random_seed=123)
        ...
        assert_almost_equal(
            ...
            mean_of_mean,
            3,
            "Mean of bootstrap does not match theoretical mean ofsampling distribution",
        )

        assert_almost_equal(
            np.std(samples), sd_of_mean, 3, "SD of bootstrap does not match theoretical SD ofsampling distribution"
        )
...
class TestGrossLev(TestCase):
    ...
    def test_gross_lev_calculation(self):
        assert_series_equal(
            Empyrical.gross_lev(self.test_pos)["2004-02-01":], self.test_gross_lev["2004-02-01":], check_names=False
        )
