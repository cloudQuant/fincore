...
def run():
    ...
    comm = ComminfoFuturesPercent(
        commission=backtest_config["commission"], margin=backtest_config["margin"], mult=backtest_config["multiplier"]
    )
    ...
    cerebro.addstrategy(
        AbberationStrategy, boll_period=params_config["boll_period"], boll_mult=params_config["boll_mult"]
    )
    ...
    cerebro.addobserver(
        ...
        file_format="log",  # Default: log (tab-separated); optional: 'csv'
    ...
    assert abs(sharpe_ratio - (-0.207410495949062)) < 1e-6, (
        f"Expected sharpe_ratio=-0.207410495949062, got {sharpe_ratio}"
    )
    assert abs(annual_return - (-0.005304130671472128)) < 1e-6, (
        f"Expected annual_return=-0.005304130671472128, got {annual_return}"
    )
    assert abs(max_drawdown - 0.27322154569702256) < 1e-6, (
        f"Expected max_drawdown=0.27322154569702256, got {max_drawdown}"
    )
