...
for year in years:
    ...
    # Brinson 归因
    allocation = (portfolio_weights - benchmark_weights) * (asset_ret.values - asset_ret.values @ benchmark_weights)

    selection = benchmark_weights * (asset_ret.values - asset_ret.values @ benchmark_weights)

    interaction = (portfolio_weights - benchmark_weights) * (asset_ret.values - asset_ret.values @ benchmark_weights)

    total_active = allocation.sum() + selection.sum() + interaction.sum()

    print(
        f"{year:<8} {allocation.sum():>11.2%} {selection.sum():>11.2%} {interaction.sum():>11.2%} {total_active:>11.2%}"
    )
...
selection = benchmark_weights * (overall_asset_ret.values - overall_asset_ret.values @ benchmark_weights)
interaction = (portfolio_weights - benchmark_weights) * (
    overall_asset_ret.values - overall_asset_ret.values @ benchmark_weights
)

print("-" * 70)
print(f"{'总体':<8} {allocation.sum():>11.2%} {selection.sum():>11.2%} {interaction.sum():>11.2%} {active_ann:>11.2%}")
...
# 胜负时的平均超额收益
win_excess = (portfolio_returns - benchmark_returns)[portfolio_returns > benchmark_returns].mean() * 252
lose_excess = (portfolio_returns - benchmark_returns)[portfolio_returns <= benchmark_returns].mean() * 252
...
try:
    ...
    ax.bar(attribution_data.keys(), attribution_data.values(), color=colors, alpha=0.8)
    ...
    out_path = out_dir / "performance_attribution.png"
    plt.savefig(out_path, dpi=100)
    print(f"\n绩效归因分析可视化已保存: {out_path}")
