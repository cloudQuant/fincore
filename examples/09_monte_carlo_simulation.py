...
returns = pd.Series(np.random.normal(0.0005, 0.015, len(dates)), index=dates, name="strategy_returns")
...
result = mc.simulate(n_paths=n_paths, horizon=horizon, seed=42)
...
for name, params in scenarios.items():
    result = mc.simulate(n_paths=1000, horizon=252, drift=params["drift"], volatility=params["volatility"], seed=42)
...
try:
    ...
    out_path = out_dir / "monte_carlo_simulation.png"
    plt.savefig(out_path, dpi=100)
    print(f"\n蒙特卡洛模拟可视化已保存: {out_path}")
