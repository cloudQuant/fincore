...
# 市场收益 (基准)
market_returns = pd.Series(np.random.normal(0.0004, 0.012, n), index=dates, name="market")

# 策略收益：有一定择时能力 (在市场上涨时超额表现)
strategy_returns = pd.Series(
    0.0003
    + 0.8 * market_returns.values
...
print("  说明: 该系数衡量策略管理人的市场时机把握能力")
print("        > 0 表示正择时能力 (市场上涨时超额收益)")
print("        < 0 表示负择时能力")
print("        ≈ 0 表示无择时能力")

if tm_timing > 0.01:
    print("  结论: 策略显示出良好的择时能力")
elif tm_timing < -0.01:
    print("  结论: 策略择时能力较弱")
else:
    print("  结论: 策略择时能力不明显")
...
print("  说明: 该模型通过二元回归评估择时能力")
print("        > 0 表示成功预测市场方向")
print("        < 0 表示市场时机把握不佳")

if hm_timing > 0:
    print("  结论: 管理人具有成功预测市场方向的能力")
else:
    print("  结论: 管理人择时能力有待提高")
...
print("\n捕获比率解读:")
print("  上行捕获率 > 100%: 策略在上涨市场中表现优于市场")
print("  下行捕获率 < 100%: 策略在下跌市场中损失小于市场")
print("  上下行比率 > 1: 整体风险调整后表现优异")
...
# 计算择时对收益的贡献
market_timing_return = timing.market_timing_return(strategy_returns, market_returns)
print(f"\n市场择时收益: {market_timing_return:.4f}")
print("  说明: 该指标量化择时决策对策略收益的贡献")

# Cornell 择时指标
try:
    cornell = timing.cornell_timing(strategy_returns, market_returns)
    print(f"\nCornell 择时指标: {cornell:.4f}")
except (ValueError, TypeError, KeyError, np.linalg.LinAlgError, FloatingPointError) as e:
    print(f"\nCornell 择时指标: 计算失败 ({e})")
...
print("\n择时能力等级:")
if timing_grade >= 80:
    print("  优秀 - 策略表现出色的择时能力")
elif timing_grade >= 60:
    print("  良好 - 策略具有一定的择时能力")
...
try:
    ...
    for i in range(window, len(strategy_returns)):
        a, b = Empyrical.alpha_beta(strategy_returns.iloc[i - window : i], market_returns.iloc[i - window : i])
