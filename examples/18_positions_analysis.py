...
# 模拟持仓数据 (单位: 元)
positions = pd.DataFrame(np.random.uniform(-500000, 1000000, (len(dates), n_assets)), index=dates, columns=assets)
...
try:
    ...
    # 2. 多空暴露
    ax = axes[0, 1]
    long_short_daily = pd.DataFrame({"多头": long_pos.sum(axis=1), "空头": short_pos.abs().sum(axis=1)})
    ...
    ax.axhline(y=daily_lev.mean(), color="r", linestyle="--", label=f"平均: {daily_lev.mean():.2f}x")
    ...
    out_path = out_dir / "positions_analysis.png"
