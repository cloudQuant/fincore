...
# 生成收益数据
returns = pd.DataFrame(np.random.multivariate_normal(mean_returns, cov_matrix, len(dates)), index=dates, columns=assets)
...
print("\n优化结果:")
print("  最优权重:")
for asset, weight in zip(assets, result_max_sharpe["weights"], strict=True):
    if weight > 0.001:
...
print("\n优化结果:")
print("  最优权重:")
for asset, weight in zip(assets, result_min_var["weights"], strict=True):
    if weight > 0.001:
        print(f"    {asset:<10} {weight:>10.2%}")
...
print("\n优化结果:")
print("  最优权重:")
for asset, weight in zip(assets, result_target_ret["weights"], strict=True):
    if weight > 0.001:
        print(f"    {asset:<10} {weight:>10.2%}")
...
print("\n优化结果:")
print("  最优权重:")
for asset, weight in zip(assets, result_target_vol["weights"], strict=True):
    if weight > 0.001:
        print(f"    {asset:<10} {weight:>10.2%}")
...
print("\n行业约束:")
print("  Technology: 30% ~ 70%")
print("  Financials: 30% ~ 70%")

print("\n优化结果:")
print("  最优权重:")
for asset, weight in zip(assets, result_sector["weights"], strict=True):
    if weight > 0.001:
        print(f"    {asset:<10} {weight:>10.2%}")
...
print("\n优化结果:")
print("  最优权重:")
for asset, weight in zip(assets, result_short["weights"], strict=True):
    if abs(weight) > 0.001:
...
comparison_df = pd.DataFrame(comparison, columns=["策略", "预期收益", "预期波动", "夏普比率"])
...
for target in target_returns:
    ...
    except (ValueError, RuntimeError, OptimizationError):  # skip infeasible target-return points
        continue
...
try:
    ...
    ax = axes[1]
    if frontier_points:
        ax.plot(frontier_df["volatility"], frontier_df["return"], "o-", label="有效前沿", markersize=4)
        # 标记各策略
        strategies_to_plot = [
        ...
        for res, name in strategies_to_plot:
            ax.plot(res["volatility"], res["expected_return"], "o", label=name, markersize=8)
